...
function monthLabel(date: string) {
  const label = MONTH_FORMAT.format(
    new Date(`${date.slice(0, 7)}-01T12:00:00Z`),
  );
...
function CalendarCell({ day }: { day: DashboardDay }) {
  ...
  const rescueTimeLabel =
    rescueOverview.totalTrackedSeconds > 0
      ...
      : "";
  const productivityIndexCSSColor =
    productivityIndex != null
      ? productivityIndexColor(productivityIndex)
      : undefined;
  return (
    <Link
    ...
    >
            ...
            {productivityIndex != null && (
              <b
                ...
                style={{
                  ...
                  background: `radial-gradient(circle, white 20%, ${productivityIndexCSSColor} 90%)`,
                }}
              >
                {productivityIndex}
              </b>
      ...
      <div className="indicators" aria-label="Источники">
        <span
          className={braceletAvailable ? "on" : ""}
          title="Браслет"
          aria-label={`Браслет: ${braceletAvailable ? "данные доступны" : "нет данных"}`}
        >
          <Watch aria-hidden="true" />
        </span>
        <span
          className={welltoryAvailable ? "on" : ""}
          title="Welltory"
          aria-label={`Welltory: ${welltoryAvailable ? "данные доступны" : "нет данных"}`}
        >
          <HeartPulse aria-hidden="true" />
        </span>
        <span
          className={todoistAvailable ? "on" : ""}
          title="Todoist"
          aria-label={`Todoist: ${todoistAvailable ? "данные доступны" : "нет данных"}`}
        >
          <ListTodo aria-hidden="true" />
        </span>
        <span
          className={rescuetimeAvailable ? "on" : ""}
          title="RescueTime"
          aria-label={`RescueTime: ${rescuetimeAvailable ? "данные доступны" : "нет данных"}`}
        >
      ...
      >
        ...
        <div className="activity-detail-grid">
          ...
          <section className="panel rescuetime-panel">
            ...
            {rescueOverview.totalTrackedSeconds > 0 ? (
              <div className="rescuetime-overview">
                <div className="productivity-summary">
                        ...
                        {rescueOverview.productivity.map((level) => (
                          <li key={level.label}>
                            <i
                              style={{
                                background:
                                  PRODUCTIVITY_COLORS[level.label] ?? "#789087",
                              }}
                            />
                            <span>
                              {PRODUCTIVITY_LABELS[level.label] ?? level.name}
                            </span>
                  ...
                  <p className="note">
                    Локальный расчёт по уровням RescueTime от −2 до 2.
                  </p>
                </div>
                  ...
                  {rescueOverview.categories
                    ...
                    .map((category, index) => (
                      <div className="activity-rank" key={category.label}>
                        <div>
                          <span>
                            <b>{Math.round(category.percentage)}%</b>{" "}
                            {category.label}
                          </span>
              ...
              <div className="rescuetime-intervals">
                ...
                <ul className="record-list scrollable-records">
                  {rescueIntervals.map((item) => (
                    <li
                      key={`${item.timestamp}-${item.perspective}-${item.label}`}
                    >
                      <RecordTime
            ...
            <p className="note">
              Активность и продуктивность — разные классификации одного времени
              и не складываются.
            </p>
          </section>
        </div>
...
const Dashboard = observer(function Dashboard() {
  ...
  const monthGroups = (store.dashboard?.days ?? []).reduce<
    Array<{
      key: string;
      days: DashboardDay[];
    }>
